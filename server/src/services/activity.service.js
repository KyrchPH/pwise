...
export async function log({ postId = null, userId = null, userName = null, action, details = null }) {
  try {
    await query(
      'INSERT INTO post_activity_log (post_id, user_id, user_name, action, details) VALUES (?, ?, ?, ?, ?)',
      [postId, userId, userName, action, details],
    );
  } catch (err) {
