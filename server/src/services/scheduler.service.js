...
// Expire overdue scheduled posts on each n8n check so stale posts are cleared
// even while scheduled publishing is off (no enabled users).
...
// Atomically claim up to `limit` rows matching
...
/**
 * Claim up to `limit` due scheduled posts in one atomic batch (earliest first),
 * each flipped to 'posting' and returned with its own page creds + media URL.
 * Lets n8n publish a whole due slot in a single run.
...
export async function claimNextBatch({ userId = null, limit = 10 } = {}) {
  ...
  // Trim to exactly what n8n needs so page tokens are not padded with extra columns.
  return {
