...
// Planner goals. Progress is measured as GROWTH WITHIN a goal's [start_date, end_date]:
//   - posts/comments/shares/views/reactions → aggregated over posts PUBLISHED in the range
//   - followers → net gained since the goal was created (baseline captured at creation,
//     compared against the live follower count)
// Goals are workspace-wide (everyone sees them); each ties to a page via account_id.
// status/completed_at are persisted terminal states, reconciled on read here, so a
// completed goal stays completed even if engagement later drops.

export const METRICS = ['followers', 'posts', 'comments', 'shares', 'views', 'reactions'];
...
const COLS = `
  id, account_id, created_by, title, metric, period, target_value, baseline_value,
...
export async function list() {
  const rows = await query(`SELECT ${COLS} FROM planner_goals ORDER BY created_at DESC`);
  const goals = await enrichGoals(rows);
...
export async function create(user, body = {}) {
  ...
  // Followers are measured as growth since creation → snapshot the current count now.
  const baseline = data.metric === 'followers' ? await captureFollowerCount(data.account_id) : null;
  const result = await query(
    `INSERT INTO planner_goals
       (account_id, created_by, title, metric, period, target_value, baseline_value, start_date, end_date, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'ongoing')`,
    [data.account_id, user?.id ?? null, data.title, data.metric, data.period, data.target_value, baseline, data.start_date, data.end_date],
  );
  return getById(result.insertId);
}

export async function update(id, body = {}) {
  const existing = await getRawRow(id); // 404 if missing
  ...
  if (finalMetric === 'followers') {
    // Re-baseline when a goal newly becomes a followers goal (or never had one).
  ...
  if (keys.length) {
    ...
    // Any edit re-opens reconciliation so status re-derives from the new definition.
    assignments.push("status = 'ongoing'", 'completed_at = NULL');
    params.push(id);
...
export async function remove(id) {
  await getRawRow(id); // 404 if missing
  await query('DELETE FROM planner_goals WHERE id = ?', [id]);
